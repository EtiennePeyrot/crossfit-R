---
title: "# crossfit: Cross-Fitting Engine for Double/Debiased ML"
...
-   a **graph of nuisance models** (propensity scores, regressions,
    etc.),
...
-   enforces **out-of-sample** use of nuisances via K-fold
    cross-fitting,
...
-   has two modes:
    ...
    -   `mode = "predict"` → returns a cross-fitted **prediction
        function**.
...
We define a nuisance $m(x) = E[Y \mid X]$ and use the cross-fitted mean
squared error of this nuisance as our target.
...
-   returns a list with:
    ...
    -   `K`, `K_required`, `methods`, `plan` – diagnostics and
        internals.
